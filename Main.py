...
linucb_agent = LinUCB(n_actions, n_features, alpha=1)
...
for i in range(0, n_steps):
    ...
    Vector[3].iloc[i] = ((Selected_Actions.iloc[i].max() - Selected_Actions.iloc[i].nlargest(2).iloc[-1]) / (i+1)) / (1/n_actions)

    # Finding the Difference between the Best and Second Miu
    Vector[4].iloc[i] = np.abs((Predicted_Miu.iloc[i].max() - Predicted_Miu.iloc[i].nlargest(2).iloc[-1]) / (Predicted_Miu.iloc[i].max() - Predicted_Miu.iloc[i].min()))

    # Finding the Difference between the Best and Second Miu+Sigma
    Vector[5].iloc[i] = np.abs(( (Predicted_Miu.iloc[i] + Predicted_Sigma.iloc[i]).max() - (Predicted_Miu.iloc[i] + Predicted_Sigma.iloc[i]).nlargest(2).iloc[-1]) / ((Predicted_Miu.iloc[i]+Predicted_Sigma.iloc[i]).max() - (Predicted_Miu.iloc[i]+Predicted_Sigma.iloc[i]).min()))
